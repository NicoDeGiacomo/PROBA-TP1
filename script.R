...
T1_IC <- function(sample, n) {
  mean <- mean(sample)
  sup <- (1 / (-(qnorm(1-(a/2))*mean / sqrt(n)) + mean))
  inf <- (1 / ((qnorm(1-(a/2))*mean / sqrt(n)) + mean))
  return(c(inf, sup))
}
# Devuelve el extremo inferior y superior para el pivote T2
T2_IC <- function(sample, n) {
  sum <- sum(sample)
  sup <- (qchisq(1 - (a/2),2*n) / (2*sum))
  inf <- (qchisq((a/2),2*n) / (2*sum))
  return(c(inf, sup))
}

############ Comienzo del Script ############

# Defino las variables para los cálculos
k <- 5000
...
for (lambda in lambdas) {
  ...
  for (n in sizes) {
    ...
    for (m in 1:k) {
      ...
      # Calculo el intervalo de confianza para cada método
      t1_ic_result <- T1_IC(sample, n)
      t1_ic_inf <- t1_ic_result[1]
      t1_ic_sup <- t1_ic_result[2]
      
      t2_ic_result <- T2_IC(sample, n)
      t2_ic_inf <- t2_ic_result[1]
      t2_ic_sup <- t2_ic_result[2]
      
      # Para esta replicación, guardo la longitud del IC y la cobertura para cada método
    ...
    t1_results <- rbind(t1_results, list(lambda=lambda, n=n, long_ic=mean(t1_long_ic), coverage=mean(t1_cobertura)))
    t2_results <- rbind(t2_results, list(lambda=lambda, n=n, long_ic=mean(t2_long_ic), coverage=mean(t2_cobertura)))
  }
}
